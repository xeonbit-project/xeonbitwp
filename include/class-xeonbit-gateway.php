...
require_once('class-xeonbit-cryptonote.php');

class Xeonbit_Gateway extends WC_Payment_Gateway
{
    private static $_id = 'xeonbit_gateway';
    private static $_title = 'Xeonbit Gateway';
    private static $_method_title = 'Xeonbit Gateway';
    private static $_method_description = 'Xeonbit Gateway Plug-in for WooCommerce.';
    ...
    private static $use_xeonbit_price = false;
    private static $use_xeonbit_price_decimals = XEONBIT_GATEWAY_ATOMIC_UNITS;

    private static $cryptonote;
    private static $xeonbit_wallet_rpc;
    private static $xeonbit_explorer_tools;
    private static $log;
    ...
    {
        return apply_filters('woocommerce_gateway_icon', '<img src="'.XEONBIT_GATEWAY_PLUGIN_URL.'assets/images/xeonbit-icon.png"/>', $this->id);
    }

    function __construct($add_action=true)
    {
        $this->id = self::$_id;
        $this->method_title = __(self::$_method_title, 'xeonbit_gateway');
        $this->method_description = __(self::$_method_description, 'xeonbit_gateway');
        ...
        self::$address = $this->settings['xeonbit_address'];
        ...
        self::$use_xeonbit_price = $this->settings['use_xeonbit_price'] == 'yes';
        self::$use_xeonbit_price_decimals = $this->settings['use_xeonbit_price_decimals'];

        $explorer_url = self::$testnet ? XEONBIT_GATEWAY_TESTNET_EXPLORER_URL : XEONBIT_GATEWAY_MAINNET_EXPLORER_URL;
        defined('XEONBIT_GATEWAY_EXPLORER_URL') || define('XEONBIT_GATEWAY_EXPLORER_URL', $explorer_url);

        if($add_action)
            add_action('woocommerce_update_options_payment_gateways_'.$this->id, array($this, 'process_admin_options'));

        // Initialize helper classes
        self::$cryptonote = new Xeonbit_Cryptonote();
        if(self::$confirm_type == 'xeonbit-wallet-rpc') {
            require_once('class-xeonbit-wallet-rpc.php');
            self::$xeonbit_wallet_rpc = new Xeonbit_Wallet_Rpc(self::$host, self::$port);
        } else {
            require_once('class-xeonbit-explorer-tools.php');
            self::$xeonbit_explorer_tools = new Xeonbit_Explorer_Tools(self::$testnet);
        }
    ...
    {
        $this->form_fields = include 'admin/xeonbit-gateway-admin-settings.php';
    }

    public function validate_xeonbit_address_field($key,$address)
    {
        if($this->settings['confirm_type'] == 'viewkey') {
            if (strlen($address) == 95 && substr($address, 0, 1) == '4')
                if(self::$cryptonote->verify_checksum($address))
                    return $address;
            self::$_errors[] = 'Xeonbit address is invalid';
        }
        return $address;
    }
    ...
    public function admin_options()
    {
        ...
        if($confirm_type === 'xeonbit-wallet-rpc')
        ...
        include XEONBIT_GATEWAY_PLUGIN_DIR . '/templates/xeonbit-gateway/admin/settings-page.php';
    }
    ...
    public static function admin_balance_info()
    {
        ...
        $wallet_amount = self::$xeonbit_wallet_rpc->getbalance();
        $height = self::$xeonbit_wallet_rpc->getheight();
        if (!isset($wallet_amount)) {
            self::$_errors[] = 'Cannot connect to xeonbit-wallet-rpc';
            self::$log->add('Xeonbit_Payments', '[ERROR] Cannot connect to xeonbit-wallet-rpc');
            return array(
                'height' => 'Not Available',
                'balance' => 'Not Available',
                'unlocked_balance' => 'Not Available',
            ...
            return array(
                'height' => $height,
                'balance' => self::format_xeonbit($wallet_amount['balance']).' Xeonbit',
                'unlocked_balance' => self::format_xeonbit($wallet_amount['unlocked_balance']).' Xeonbit'
            );
        }
    }
    ...
    public function process_payment($order_id)
    {
        ...
        $xeonbit_amount = 1e8 * $fiat_amount / $rate;

        if(self::$discount)
            $xeonbit_amount = $xeonbit_amount - $xeonbit_amount * self::$discount / 100;

        $xeonbit_amount = intval($xeonbit_amount * XEONBIT_GATEWAY_ATOMIC_UNITS_POW);

        $query = $wpdb->prepare("INSERT INTO $table_name (order_id, payment_id, currency, rate, amount) VALUES (%d, %s, %s, %d, %d)", array($order_id, $payment_id, $currency, $rate, $xeonbit_amount));
        $wpdb->query($query);

        $order->update_status('on-hold', __('Awaiting offline payment', 'xeonbit_gateway'));
    ...
    public static function do_update_event()
    {
        ...
        if(self::$confirm_type == 'xeonbit-wallet-rpc')
            ...
            $height = self::$xeonbit_explorer_tools->getheight();
        set_transient('xeonbit_gateway_network_height', $height);

        // Get pending payments
        $table_name_1 = $wpdb->prefix.'xeonbit_gateway_quotes';
        $table_name_2 = $wpdb->prefix.'xeonbit_gateway_quotes_txids';
        ...
        foreach($pending_payments as $pending) {
            ...
            $amount_xeonbit = $quote->amount_total;
            ...
            $paid = $amount_paid > $amount_xeonbit - XEONBIT_GATEWAY_ATOMIC_UNIT_THRESHOLD;
            ...
            if($paid && $confirmed) {
                self::$log->add('Xeonbit_Payments', "[SUCCESS] Payment has been confirmed for order id $order_id and payment id $payment_id");
                ...
                if(self::is_virtual_in_cart($order_id) == true){
                    $order->update_status('completed', __('Payment has been received.', 'xeonbit_gateway'));
                } else {
                    $order->update_status('processing', __('Payment has been received.', 'xeonbit_gateway'));
                }

            } else if($paid) {
                self::$log->add('Xeonbit_Payments', "[SUCCESS] Payment has been received for order id $order_id and payment id $payment_id");
                ...
                if($order_age_seconds > self::$valid_time) {
                    self::$log->add('Xeonbit_Payments', "[FAILED] Payment has expired for order id $order_id and payment id $payment_id");
                    $query = $wpdb->prepare("UPDATE $table_name_1 SET pending=0 WHERE payment_id=%s", array($payment_id));
                    $wpdb->query($query);

                    unset(self::$payment_details[$order_id]);

                    $order->update_status('cancelled', __('Payment has expired.', 'xeonbit_gateway'));
                }
            }
        }
    }
    ...
    {
        $txs = array();
        $payments = self::$xeonbit_wallet_rpc->get_all_payments($payment_id);
    ...
    {
        $txs = array();
        $outputs = self::$xeonbit_explorer_tools->get_outputs(self::$address, self::$viewkey);
        ...
        if (count($details)) {
            ...
            foreach($details as $tx) {
                ...
                $txs[] = array(
                    ...
                    'amount_formatted' => self::format_xeonbit($tx->amount_paid)
                );
            ...
            if(count($heights) && !in_array(0, $heights)) {
                $height = get_transient('xeonbit_gateway_network_height');
                $highest_block = max($heights);
            ...
            $time_to_confirm = self::format_seconds_to_time($blocks_to_confirm * XEONBIT_GATEWAY_DIFFICULTY_TARGET);
            ...
            if(self::$confirm_type == 'xeonbit-wallet-rpc') {
                $array_integrated_address = self::$xeonbit_wallet_rpc->make_integrated_address($payment_id);
                if (isset($array_integrated_address['integrated_address'])) {
                    $integrated_addr = $array_integrated_address['integrated_address'];
                } else {
                    self::$log->add('Xeonbit_Gateway', '[ERROR] Unable get integrated address');
                ...
                if ($address) {
                    ...
                    self::$log->add('Xeonbit_Gateway', '[ERROR] Merchant has not set Xeonbit address');
                    return '[ERROR] Merchant has not set Xeonbit address';
                }
            }
            ...
            $amount_formatted = self::format_xeonbit($amount_due);
            $qrcode_uri = 'xeonbit:'.$address.'?tx_amount='.$amount_formatted.'&tx_payment_id='.$payment_id;
            ...
            $payment_details = array(
                ...
                'amount_total_formatted' => self::format_xeonbit($amount_total),
                'amount_paid_formatted' => self::format_xeonbit($amount_paid),
                'amount_due_formatted' => self::format_xeonbit($amount_due),
                'status' => $status,
    ...
    public static function get_payment_details_ajax() {
        ...
        if($order->get_payment_method() != self::$_id)
            self::ajax_output(array('error' => '[ERROR] Order not paid for with Xeonbit'));
    ...
    public static function admin_order_page($post)
    {
        ...
        if(!is_array($details)) {
            $error = $details;
            include XEONBIT_GATEWAY_PLUGIN_DIR . '/templates/xeonbit-gateway/admin/order-history-error-page.php';
            return;
        }
        include XEONBIT_GATEWAY_PLUGIN_DIR . '/templates/xeonbit-gateway/admin/order-history-page.php';
    }
    ...
    public static function customer_order_page($order)
    {
        ...
        if(!is_array($details)) {
            $error = $details;
            include XEONBIT_GATEWAY_PLUGIN_DIR . '/templates/xeonbit-gateway/customer/order-error-page.php';
            return;
        }
        $show_qr = self::$show_qr;
        $details_json = json_encode($details);
        $ajax_url = WC_AJAX::get_endpoint('xeonbit_gateway_payment_details');
        include XEONBIT_GATEWAY_PLUGIN_DIR . '/templates/xeonbit-gateway/customer/order-page.php';
    }
    ...
    public static function customer_order_email($order)
    {
        ...
        $details = self::get_payment_details($order_id);
        if(!is_array($details)) {
            include XEONBIT_GATEWAY_PLUGIN_DIR . '/templates/xeonbit-gateway/customer/order-email-error-block.php';
            return;
        }
        include XEONBIT_GATEWAY_PLUGIN_DIR . '/templates/xeonbit-gateway/customer/order-email-block.php';
    }
    ...
    public static function use_xeonbit_price()
    {
        return self::$use_xeonbit_price;
    }
    ...
    {
        $rate = self::get_live_rate($currency);
        $xeonbit_amount = intval(XEONBIT_GATEWAY_ATOMIC_UNITS_POW * 1e8 * $price / $rate) / XEONBIT_GATEWAY_ATOMIC_UNITS_POW;
        $xeonbit_amount_formatted = sprintf('%.'.self::$use_xeonbit_price_decimals.'f', $xeonbit_amount);

        return <<<HTML
            <span class="woocommerce-Price-amount amount" data-price="$price" data-currency="$currency"
        data-rate="$rate" data-rate-type="live">
            $xeonbit_amount_formatted
            <span class="woocommerce-Price-currencySymbol">XNB</span>
        </span>
...
HTML;
    ...
    public static function convert_wc_price_order($price_html, $order)
    {
        ...
        $xeonbit_amount = intval(XEONBIT_GATEWAY_ATOMIC_UNITS_POW * 1e8 * $price / $rate) / XEONBIT_GATEWAY_ATOMIC_UNITS_POW;
        $xeonbit_amount_formatted = sprintf('%.'.XEONBIT_GATEWAY_ATOMIC_UNITS.'f', $xeonbit_amount);
...
HTML;
    ...
    public static function format_xeonbit($atomic_units) {
        return sprintf(XEONBIT_GATEWAY_ATOMIC_UNITS_SPRINTF, $atomic_units / XEONBIT_GATEWAY_ATOMIC_UNITS_POW);
    }
