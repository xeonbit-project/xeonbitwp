 ...
 * http://implix.com
 * Modified to work with xeonbit-rpc wallet by Serhack and cryptochangements
 * Modified to work with xeonbit-wallet-rpc wallet by mosu-forge
 */

defined( 'ABSPATH' ) || exit;

class Xeonbit_Wallet_Rpc
{
    ...
    protected function & getResponse(&$pRequest)
    {
        ...
        if (0 < curl_errno($ch)) {
            if(is_admin())
                echo '[ERROR] Failed to connect to xeonbit-wallet-rpc at ' . $this->host . ' port '. $this->port .'</br>';
        }
    ...
    /*
     * The following functions can all be called to interact with the Xeonbit RPC wallet
    ...
    public function make_uri($address, $amount, $recipient_name = null, $description = null)
    {
        ...
        $new_amount = $amount * XEONBIT_GATEWAY_ATOMIC_UNITS_POW;
